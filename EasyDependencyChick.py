...
def change_language(*args):
    ...
    labels = {
        'English': ["Jar File:", "Report Path:", "Vulnerability DB Update:", "NVD API Key:", "Logs:", "Scan", "Language:"],
        '简体中文': ["Jar 文件:", "报告路径:", "漏洞库更新:", "NVD API 密钥:", "日志:", "扫描", "语言:"],
        '繁體中文': ["Jar 檔案:", "報告路徑:", "漏洞庫更新:", "NVD API 金鑰:", "日誌:", "掃描", "語言:"],
        '日本語': ["Jar ファイル:", "レポートパス:", "脆弱性DB更新:", "NVD APIキー:", "ログ:", "スキャン", "言語:"]
    }
    label_jar.config(text=labels[language][0])
    label_report.config(text=labels[language][1])
    label_update.config(text=labels[language][2])
    label_nvd_key.config(text=labels[language][3])
    label_logs.config(text=labels[language][4])
    scan_button.config(text=labels[language][5])
    label_language.config(text=labels[language][6])
...
label_update.grid(row=2, column=0, sticky=tk.W)
update_var = tk.StringVar(value="No")
ttk.Radiobutton(mainframe, text="No", variable=update_var, value="No").grid(row=2, column=1, sticky=tk.W)
ttk.Radiobutton(mainframe, text="Yes", variable=update_var, value="Yes").grid(row=2, column=2, sticky=tk.W)

# NVD API Key (only visible if update is Yes)
label_nvd_key = ttk.Label(mainframe, text="NVD API Key:")
label_nvd_key.grid(row=3, column=0, sticky=tk.W)
entry_nvd_key = ttk.Entry(mainframe, width=50)
entry_nvd_key.grid(row=3, column=1)
entry_nvd_key.grid_remove()

# Log Text
label_logs = ttk.Label(mainframe, text="Logs:")
label_logs.grid(row=4, column=0, sticky=tk.W)
log_text = tk.Text(mainframe, height=10, width=70)
log_text.grid(row=5, columnspan=3, pady=10, sticky=(tk.W, tk.E))
log_text.config(state=tk.NORMAL)

# Progress Bar
progress_bar = ttk.Progressbar(mainframe, mode='indeterminate')
progress_bar.grid(row=6, columnspan=3, pady=10, sticky=(tk.W, tk.E))
progress_bar.grid_remove()

# Scan Button
scan_button = ttk.Button(mainframe, text="Scan", command=run_scan)
scan_button.grid(row=7, column=1)
scan_button.config(state=tk.DISABLED)
...
label_language.grid(row=8, column=0, sticky=tk.W, pady=(10,0))
language_var = tk.StringVar(value="English")
language_option = ttk.Combobox(mainframe, textvariable=language_var, values=["English", "简体中文", "繁體中文", "日本語"])
language_option.grid(row=8, column=1, pady=(10,0))
